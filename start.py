...
'''
#     然后,你可以一步一步的来判断我提交给你的函数,是否是过滤函数.
#     step 1: 如果内容开头包含函数相关的注释,可以先看注释来大体判断函数的用途.
#     step 2: 检查函数的函数名,如果函数名称为sqlfilter checkFileName这种明显的函数名,那么其大概率是过滤函数
#     step 3: 检查函数的函数逻辑,这一步是最关键的. 需要按照先验只是来对我提交的内容进行判断.
#
#     按照这个思维链路,一步一步的进行消毒函数判断,接下来我会给你
